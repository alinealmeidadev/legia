...
export function ProcessEditDialog({
  ...
  const [formData, setFormData] = useState({
    client_id: process.client_id.toString(),
    process_type: process.process_type,
    title: process.title,
    description: process.description || '',
    status: process.status,
    priority: process.priority,
    estimated_days: process.estimated_days.toString(),
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch(`/processes/${process.id}`, {
        ...formData,
      ...
      toast({
        title: 'Erro',
        description: error.response?.data?.detail || 'Erro ao atualizar processo',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: 'Erro',
        description: error.response?.data?.detail || 'Erro ao excluir processo',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
